...
#define BH1750_CMD_CONTHIRES 0x10 
...
static esp_err_t bh1750_init(void) {
    ...
    i2c_master_write_byte(cmd, BH1750_CMD_POWERON, true);
    i2c_master_stop(cmd);
    ...
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (BH1750_SENSOR_ADDR << 1) | I2C_MASTER_WRITE, true);
    i2c_master_write_byte(cmd, BH1750_CMD_CONTHIRES, true);
    i2c_master_stop(cmd);
...
// Hàm đọc giá trị cường độ ánh sáng Lux thực tế từ BH1750
...
static void set_parent_to_candidate(const mesh_parent_t *cand)
{
    ...
    p.sta.threshold.authmode = WIFI_AUTH_OPEN;           
    memset(p.sta.password, 0, sizeof(p.sta.password));   
...
static void send_sensor_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (lux < 0) {
            ESP_LOGE(TAG, "BH1750 read error!");
            lux = 0.0f; 
        }
        ESP_LOGI(TAG, "Light BH1750: %.2f Lux", lux);
        ...
        snprintf(line, sizeof(line), "Light:%.1fLx", lux); ssd1306_display_text(&oled, 4, line, false);
